...
- emotion: 이 프로젝트에서 사용빈도수가 낮지만
- chakra-ui : 최근에 알게된 ui 컴포넌트. @chakra-ui/react 66.5 KB로 material-ui core 5.97 MB 보다 가벼우며, material ui보다 props 사용이 직관적이며 로직처리가 쉽다. 오히려 직접 ui를 개발하는 디벨로퍼들에게 ui구현 부분의 자율성을 줍니다.
- Typescript: 정적 코드 분석과 품질을 위해 사용.
- React Router Dom: spa 페이지 간의 이동을 위해 사용하였습니다.

## Challenge 설명 및 고민한 로직

#### 구현할 서비스의 특성을 고려한 툴 고르기.
...
##### 옵션 선택을 하지 않은 경우 유효성 검사로 alert 띄우기
...
- 상품 디테일페이지에서 `Dangerously Set innerHTML` 를 사용하게 되면서 xss 공격의 대응을 위한 대응이 필요합니다.

  - ` eslint`, `eslint-plugin-jam3` 사용
